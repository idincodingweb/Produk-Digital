...
const BestsellersPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // ambil 3 data terakhir sebagai best seller
        const bestSellerData = allEbooks;
        setBestsellers(bestSellerData);
        setFilteredEbooks(bestSellerData);
      } catch (error) {
  ...
  const handleSearch = (e) => {
    ...
    setSearchTerm(term);
    const filtered = bestsellers.filter(
      (ebook) =>
  ...
  return (
    <section className="py-16 bg-gray-50">
      <div className="container mx-auto px-4">
        <Link
          to="/"
          className="inline-flex items-center mb-4 text-purple-600 hover:text-purple-800"
        >
          <FontAwesomeIcon icon={faArrowLeft} className="mr-2" />
          Kembali
        </Link>
        <h2 className="text-3xl font-bold text-center mb-8">Semua Produk Terlaris</h2>
